...
def transform_team_identity(team, competition_id=None):
    return {
        "team_id": team.get("id"),
        "name": team.get("name"),
...
def transform_teams(raw_teams_data):
    ...
    competition_id = raw_teams_data.get("competition", {}).get("id")
    rows = [transform_team_identity(team, competition_id) for team in teams]
